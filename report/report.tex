...
Once we implemented and trained the proposition of \cite{egornn} and \cite{sparnet}, it was clear that the \textit{Ego-Rnn} approach had 2 main problems : scarce efficiency of the flow network, and lack of motion information extracted in the appearance network, in which the CAM often risulted on focusing just on one static object on the foreground. The
\textit{Sparnet} approach improved this behaviour, redirecting the focus were the motion
really is. At this point we saw how the network mainly had 2 points of failure : precision in isolating the motion location, and encoding the temporal evolution of the motion(e.g. failed to recognize open/close actions). To improve the precision of the motion localization, we argue that moving the Motion Segmentation task to the \textit{conv4} output of our backbone can help the network. In this way it can leverage 14x14 higher resolution motion maps, and using lower-level features can better capture relationship in the motion of multiple objects. The task was implemented as a regression with the ground truths as a grayscales obtained from the mean of neighbouring pixels. This achieved a 10 percent boost on performances on the baseline using only 7 frames.

This was about the precision. To address the problem of forcing the network to include motion clues, we avoided using optical flow for this purpose following the work of \cite{flow_cit_1}.
So we propose 2 strategies : one that acts on the CNN, trying to extract motion information from the sequence of RGB frames; and one that acts on the convLSTM module, leaving the backbone focus on apperance and teaching the convLSTM to track in its memory features that reflect the motion changes in the appearance. The first approach stems from the work of \cite{reprflow} and includes
its proposed Represenation Flow layer, which is a layer that estimates optical flow from the sequence of feature maps in the middle of a CNN. As said before the idea behind this
is that the CAM alone without clues about what is moving can wind up focusing on static objects that the network is trained to recognize. Including this information about the motion allows us to remove the CAM and use this features to represent the motion, and send them to the convLSTM module which will encode the temporal evolution.

Our last proposition, as anticipated focus on the convLSTM module, and involves adding a discriminator to the convLSTM module, training to discern between series of static frames, and actual frames from a video sequence. This should encourage the spatio-temporal encoding to neglect features that are do not contain useful information 
about motion in the input frames.

These last 2 architectures did not provide a solid performance boost as the first one did, but they stem from valid ideas and so they were worth mentioning. 
...
We have replicated some of the same experiments of \cite{egornn} on the original egornn. We have run each of these experiments three times and then we have averaged the results.
...
The wrong focus of the CAMs  highly justifies the implementation of the self-supervised task which exploits the motion maps. First of all we have replicated the same experiment of \cite{sparnet} (the one in which \textit{egornn} is the Action Recognition Block). The hyperparameters used are:
...
We have also replicated this experiment as a regression problem.
...
Also in this case the highest value of the accuracy is obtained for 16 frames. The first stage is exactly the same so we have not replicated it three times more. For the second stage we observe that with this combination of values for the hyperparameters the regression performs more than 4 points better than classification with 16 frames, while the accuracies are roughly the same when the number of frames is 7.
...
task, and some other propositions like in \cite{reprflow}. The summary of our results is shown in \reff{summary_table}.
