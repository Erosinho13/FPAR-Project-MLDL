...
class AttentionModelMS(nn.Module):
    def __init__(self, num_classes = 61, mem_size = 512):
        ...
        self.fc_SD = nn.Linear(mem_size, 2)
        
        self.classifier = nn.Sequential(self.dropout, self.fc)
        self.classifier_SD = nn.Sequential(self.dropout, self.fc_SD)
        
        self.msBlock = msblock()

        
    def forward(self, inputVariable, no_cam = False, mmaps = False, static_frames = False):
        
        if mmaps and static_frames:
            raise Exception("If you pass the mmaps you can't use the SD_block")
        ...
        if not static_frames:
            ...
            feats = self.classifier_SD(feats1)
